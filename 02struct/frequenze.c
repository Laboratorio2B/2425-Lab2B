...
int main(int argc, char *argv[])
{
  ...
  // inserisce le stringhe passate sulla linea di comando nell'abr
  // se sono già presenti incrementa il contatore associato 
  strint *root = NULL;
  for(int i=1;i<argc;i++) {
    // cerca se la stringa è gia presente
    strint *nodo = abr_ricerca(root,argv[i]);
    if(nodo!=NULL)   // se è presente incrementa intero associato
      nodo->n +=1;
    else {
      strint *si = strint_crea(argv[i],1);
